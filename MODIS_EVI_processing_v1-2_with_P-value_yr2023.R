...
EVIFileName = 'D01_BART_FB_Detailed_VIq02_MOD13Q1_500m_2023'
QAbits = '01'
startyear <- 2003
sensor = 'TERRA_AQUA'
#sensor = 'TERRA'
...
for (DOY in c(1,17,25,33,41,49,57,65,73,81,89,97,105,113,121,129,137,145,153,161,169,177,185,193,201,209,217,225,233,241,249,257,265,273,281,289,297,305,313,321,329,337,345,353,361)) {
 # Filter to specified year
  ...
  #Remove the first year if the paired groups are unbalanced (need the same number of years in group A and group )
  #df_EVI_DOY <- df_EVI_DOY[-c(1),]
  
  
  # Perform linear regression
  model <- lm(df_EVI_DOY$FITTED ~ df_EVI_DOY$YEAR)
  
  # Extract the coefficient for the slope
  slope <- coef(model)[[2]]
  
  # Perform t-test
  t_test <- summary(model)$coefficients["df_EVI_DOY$YEAR", ]
  ...
  conf_interval <- confint(model)["df_EVI_DOY$YEAR", ]
  ...
  # Stratify into two groups, 2004-2012, 2013-2022, for two-sample T-test
  ...
  # Compute linear regression model for DOYs across all years (2003-2022)
  ...
  TTest3 <- TTest3[-c(1),] 
  #Save LM P.value in new dataframe
  TTest4 <- data.frame("Full.array.P.value"=TTest3$TTEST.Pr...t..) 
  #Add LM P.value to new column in data frame
  PTtest2$Full.array.P.value <- TTest4$Full.array.P.value 
  
  PTtest2$T_test_all_years <- t_value
  PTtest2$P_value_all_years <- p_value
  PTtest2$conf_int_lower <- conf_interval[1]
  PTtest2$conf_int_upper <- conf_interval[2]
...
write.csv(df_EVI_final_spread, file = (sprintf("~/R_Scripts/MODIS/data_out/%s_MODIS_%s_EVI_QAbit_%s_Final%s_SPAN_%s_t-test.csv",EVIFileName, sensor, QAbits, YEAR, loessSPAN)))

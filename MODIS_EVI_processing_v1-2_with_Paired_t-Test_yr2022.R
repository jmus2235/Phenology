...
EVIFileName = 'D17_TEAK_FB_EN_Detailed_VIq02_MOD13Q1_2022'
QAbits = '01'
startyear <- 2003
sensor = 'TERRA_AQUA'
#sensor = 'TERRA'
...
for (DOY in c(1,17,25,33,41,49,57,65,73,81,89,97,105,113,121,129,137,145,153,161,169,177,185,193,201,209,217,225,233,241,249,257,265,273,281,289,297,305,313,321,329,337,345,353,361)) {
 # Filter to specified year
  ...
  # Compute t-test for unequal variances between two time series (2003-2011 and 2012-2021)
  PTtest <- t.test(FITTED ~ GROUP, data = df_EVI_DOY, paired = TRUE, var.equal= FALSE)
  ...
  # Compute linear model for each DOY across all years (2003-2021)
